...
2. **Постановка задачи оптимизации**  
   Задача состоит в нахождении весов $w$, которые минимизируют невязку между предсказанными и целевыми значениями $WMAE$. Каждое уравнение системы выглядит следующим образом: $$\frac{\sum_{j=0}^6 w_j \cdot |j - k|}{\sum_{j=0}^6 w_j} = WMAE[k], \quad k = 0, 1, \dots, 6.$$
...
4. **Результаты оптимизации**  
   После подбора весов было достигнуто хорошее соответствие между целевыми и аппроксимированными значениями \(WMAE\). Итоговый график демонстрирует сравнение:  

   ![График](# "вставить график проверки приближения WMAE").
---
### **Сплит данных**
