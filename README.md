...
Далее вероятности, предсказанные бинарными классификаторами, используются для обучения дополнительной регрессионной модели, которая определяет итоговый класс. В качестве такой модели был выбран **CatBoostRegressor**, обученный с использованием функции потерь MAE, для обучения которой использовалось два типа весов: одни получены из дисбаланса классов (Class Weights в таблице результатов), а другие подбирались (см. пункт ниже).

---
### **Подбор весов для CatBoostRegressor**
Итоговая метрика соревнованяи WMAE, веса которой неизвестны. В данном пункте предложен алгоритм подбора весов с использованием результатов лидерборда.
...
Для каждой группы была рассчитана суммарная значимость путём сложения важностей её признаков. Это позволило оценить вклад каждой категории признаков в итоговую модель. Важно отметить, что некоторые признаки могли не войти в итоговый анализ из-за удаления сильно коррелированных или содержащих много пропусков признаков.
