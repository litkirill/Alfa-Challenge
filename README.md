...
Важность каждого признака была рассчитана с использованием встроенного метода FeatureImportance в CatBoost. Затем признаки были сгруппированы в соответствии с заранее заданной классификацией, описанной в разделе «Генерация признаков и их обработка». Для каждой группы важность её признаков была суммирована, чтобы получить общую значимость группы.

На графике ниже показаны результаты агрегации, которые позволяют визуально оценить вклад каждой группы в итоговую модель.
