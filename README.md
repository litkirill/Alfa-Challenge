...
2. **Постановка задачи оптимизации**  
   Задача заключается в определении весов \(w\), которые минимизируют разницу между предсказанными и целевыми значениями $WMAE$. Для каждого класса $k$; уравнение принимает следующий вид:  $\frac{\sum_{j=0}^6 w_j \cdot |j - k|}{\sum_{j=0}^6 w_j} = WMAE[k], \quad k = 0, 1, \dots, 6.$ Здесь левая часть выражает средневзвешенное расстояние между классами, которое должно совпадать с целевыми значениями $WMAE$.
